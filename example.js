...
async function example() { // manual session
  ...
  for (let i = 0; 1e3 > i; ++i) p.push(httpClient.get(url, { headers, sessionKey }));
...
function example2() { // auto session
  return httpClient.get(url, { headers: { Foo: 1, bar: 2 } }).then(({ statusCode, data }) => log(statusCode, data.length)).then(() => {
    const p = [];
    for (let i = 0; 1e3 > i; ++i) p.push(httpClient.get(url, { headers: { Foo: 1, bar: 2 } })); // session will be derived from the previous one
    return Promise.all(p);
  });
}
